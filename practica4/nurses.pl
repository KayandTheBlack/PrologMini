...
% worksNH-N-H          "nurse N works  at hour H" 	EX: worksNH-nurse07-5
% nurseType-N-T     "nurse N has type T"        (if Type=1: WWWRWWW, if Type=2: WWWRRWWW)
%  	NOTE: variables N are in truth nurseXX, and not just a number
...
needed(H,N):- needs(L), member(H-N,L). %This predicate gives all the needs in order, backtrack needed for all.

%This predicate gives 'all' hours nurse is working in H, but needs to backtrack to tell all.
workingHourForTypeAndStartH(1,StartH,H):- between(0,1,I), H is (StartH+I) mod 24. %Type=1: WWRWWRWW
...
workingHourForTypeAndStartH(2,StartH,H):- between(0,1,I), H is (StartH+I) mod 24. %Type=2: WWRRWWRWW
...
workingHourForTypeAndStartH(3,StartH,H):- between(0,1,I), H is (StartH+I) mod 24. %Type=3: WWRWWRRWW
...
workingHourForTypeAndStartH(4,StartH,H):- between(0,1,I), H is (StartH+I) mod 24. %Type=4: WWRRWWRRWW
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%MODIFICAR ESTE TROZO:
writeClauses:-
    eachNurseOneStartHour,
	eachNurseOneType,
	startHandTypeCreatesWorkHours,
	startHandTypeCreatesNotWorkHours,
	eachNurseNotBlocked,
	needsCovered,
    true.

eachNurseOneStartHour:- nurse(N), findall(startsNH-N-H, hour(H), Lits), exactly(1,Lits),fail.
eachNurseOneStartHour.

eachNurseOneType:- nurse(N), findall(nurseType-N-T, type(T), Lits), exactly(1,Lits),fail.
eachNurseOneType.

needsCovered:- needed(H, MIN), findall(worksNH-N-H, nurse(N), Lits), atLeast(MIN, Lits), fail.
needsCovered.

eachNurseNotBlocked:- nurseIDandBlocking(N, SB, FB), hourInRange(SB,FB,H), 
	negate(worksNH-N-H, C), writeClause([C]), fail. % worksNH-N-H, hourInRange(SB, FB, H), Lits, negateAll(Lits,Nlits),
eachNurseNotBlocked.

notmember(X,L):- member(X,L), !, fail.
notmember(_,_).

startHandTypeCreatesWorkHours:- nurse(N), hour(SH), type(T), 
	workingHourForTypeAndStartH(T,SH,H), negate(startsNH-N-SH,L0), negate(nurseType-N-T,L1),
	writeClause([L0,L1, worksNH-N-H]), fail.
startHandTypeCreatesWorkHours.


startHandTypeCreatesNotWorkHours:- nurse(N), hour(SH), type(T),
	findall(H, workingHourForTypeAndStartH(T,SH,H), TrulyWorking),
	hour(H1), notmember(H1,TrulyWorking),
	negate(startsNH-N-SH,L0), negate(nurseType-N-T,L1), negate(worksNH-N-H1, L2), writeClause([L0,L1,L2]), fail.
startHandTypeCreatesNotWorkHours.
% =====================================================================
...
% escribe la clausulas que codifican la formula    Var <--> Or(Lits)
