...
filledInputValues.
%nth1 retorna Iesim element del segon parametre en el 3r parametre 

eachIJexactlyOneK:- row(I), col(J), findall( x-I-J-K, val(K), Lits ), exactly(1,Lits), fail.
eachIJexactlyOneK. %REMEMBER findall: put in third all elements like first param that fulfill 2nd param.
...
eachBlockEachKexactlyOnce:- blockID(Iid,Jid), %returns 0-0, 0-1... 2-2.
        val(K), findall( x-I-J-K, squareOfBlock(Iid,Jid,I,J), Lits ), exactly(1,Lits), fail.
	%Idea, for each block and value, put all of those in Lits (all x-I-J-sameValue s.t I J belong to same block), and ensure just one.
