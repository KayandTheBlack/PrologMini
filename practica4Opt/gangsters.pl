...
writeClauses(K):- 
	...
	eachGangsterNotBlocked,
    eachHourMinGangsters,
	noTwoConsecutiveDifferentTasks,
...
eachHourEachGangsterAtMostOneTask.
eachGangsterNotBlocked:- blocked(G,H), task(T), negate(does-G-T-H, L), writeClause([L]), fail.
eachGangsterNotBlocked.
eachHourMinGangsters:- task(T), hour(H), gangstersNeeded(T,L), nth1(H,L,MIN), 
	findall(does-G-T-H, gangster(G), Lits), atLeast(MIN, Lits), fail.
eachHourMinGangsters.

noTwoConsecutiveDifferentTasks:- gangster(G), hour(H), \+ H = 72, task(T), task(T2), \+ T2 = T, H2 is H+1,
	negate(does-G-T-H, L1), negate(does-G-T2-H2, L2), writeClause([L1,L2]), fail.
noTwoConsecutiveDifferentTasks.

eachGangsterAtMostKConsecutiveHours(K):- gangster(G), hour(H), Hend is H+K, Hend =< 72, task(T),
	findall(does-G-T-H2, between(H,Hend, H2), Lits), negateAll(Lits, C), writeClause(C), fail.
eachGangsterAtMostKConsecutiveHours(_).

%Idea behind this: given a list of true lits (model), compute max number of consecutive hours.
generateListMaxKforGangster(_, [], N):- gangsters(GL), length(GL, N2), N > N2, !.
generateListMaxKforGangster(M, [X|L], N):-	
	gangster(GL), nth1(N, GL, G),
	maxKforGangster(G, X, 0, 0, 1,  M),
	N is N+1, generateListMaxKforGangster(M, L, N1).
%% gangster, maxK, K until now, hour to check, model
works(G, H, M):-
	task(T), member(does-G-T-H, M).

max(A,B,A):- A>=B, !.
max(A,B,B).
maxKforGangster(_, K, Ki, Kimax, H, _):- \+ hour(H), !, max(Ki, Kimax, K).
maxKforGangster(G, K, Ki, Kimax, H, M):- %Caso que trabaja: .... need to end!
	works(G, H, M), !, 
	Ki2 is Ki+1, H2 is H+1,
	maxKforGangster(G, K, Ki2, Kimax, H2, M).
maxKforGangster(G, K, Ki, Kimax, H, M):-
	H2 is H+1, max(Ki, Kimax, MAX),maxKforGangster(G, K, 0, MAX, H2, M).

maxConsecutiveHours(M,K):-
	generateListMaxKforGangster(M, L, 1), write(L).
	%maxKforGangster(g12, X, 0, 0, 1,  M), write(X).
