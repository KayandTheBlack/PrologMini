...
%1 10 4 14 12 3 11 15 2 5 9 17 20 18 16 8 7 13 19 6 1
% 1          2       3 4      5          6       7 8
...
% for this one i use too cost-i meaning transition from position i-1 to i had a cost!
...
writeClauses(K):-
    ...
    costGeneration,
    costNotAbove(K),
    exactlyOneNodePerPosition,
    firstNodeExactlyTwice,
    eachNodeExactlyOnceMinusFirst.



init:- 
	numNodes(N),  writeClause([ visited-1-0 ]), writeClause([ visited-1-N ]).
init.
visit:-
	node(X), findall(visited-X-P, position(P), Lits), atLeast(1, Lits),fail.
visit.

adjacent(N1,N2):-
	adjacency(N1,L), member(N2,L).
noTrajectIfThereIsNoCorrespondingEdge:-
	position(P), P \= 0, P1 is P-1, node(N), findall(visited-N2-P1, adjacent(N2,N), L2),
	writeClause([\+visited-N-P | L2]), fail.
noTrajectIfThereIsNoCorrespondingEdge.

costGeneration:-
	position(P1), numNodes(N), P1\= N, P2 is P1+1, node(N1), node(N2), 
	Parity is (N1+N2) mod 2, Parity = 1, writeClause([\+visited-N1-P1, \+visited-N2-P2, cost-P2]), fail.
costGeneration.
%idea: no two incongruent 0 mod 2 or cost!

costNotAbove(K):-
	findall(cost-P,position(P),L), atMost(K, L).

exactlyOneNodePerPosition:-
	position(P), findall(visited-N-P, node(N), Lits), exactly(1,Lits), fail.
exactlyOneNodePerPosition.

firstNodeExactlyTwice:-
	numNodes(N), position(P), P\=0, P\=N, writeClause([\+visited-1-P]),fail.
firstNodeExactlyTwice.

eachNodeExactlyOnceMinusFirst:-
	node(N), N\=1, findall(visited-N-P, position(P), L), exactly(1,L),fail.
eachNodeExactlyOnceMinusFirst.

%no segundo o entonces uno de los primeros (excepto 1)


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Prolog predicates to be implemented:    

% Given a model M, Cost is its cost:		DONE
cost(M,Cost):-
	findall(cost-I, member(cost-I,M), L), length(L, Cost).

%show the solution: 				DONE
displaySol(M):-
	printSol(0,M),nl.

printSol(N,M):-
	member(visited-X-N, M), write(X), write(' '), N1 is N+1, printSol(N1,M),!.
printSol(X,_):-
	numNodes(X1), X is X1+1.
